...
02 CommonJS规范
image-20220210101652166image-20220210101720533
03 modules模块化规范写法
...
02-2 url模块 新版用法
http://www.nodejs.com.cn/api/url.html#new-urlinput-base

- `input` <string> 要解析的绝对或相对的输入网址。 如果 input 是相对的(`/home`)，则需要 base。 如果 input 是绝对的(`http://www.a.com/home`)，则忽略 base。 如果 input 不是字符串，则先转换成字符串。

- `base` <string> 如果 input 不是绝对的，则为要解析的基本网址(`base就是域名 http:127.0.0.1:3000`)。 如果 base 不是字符串，则先转换成字符串。

- 通过相对于 base 解析 input 来创建新的 URL 对象。 如果 base 作为字符串传入，则其将被解析为等效于 new URL(base)。
02-2.1 new URL(input[, base])
...
// 获取 url中？后面的参数
myURL.searchParam;   // 这是一个URLSearchParams对象，可以使用 for...of... 进行迭代；也可以使用 .get(key) 来获取值。
解析绝对地址
...
url.searchParams 是一个迭代器，可以使用 for...of 进行迭代；也可以使用 get 方法获取属性值。
...
// id 8
// name mouse
02-2.2 URLSearchParams(string | obj | iterable)

将 form 表单的字符串转化成 URLSearchParams 对象。 前导 '?'（如果存在）将被忽略。
反之，toString() 可以将 URLSearchParams 对象转成字符串；.toString() 是隐式调用的
...
new URLSearchParams(iterable), iterable 可以是 Array 或任何可迭代对象。
...
** 02-2.3 url.format(URL[, options])**

options
-- auth <默认true> 是否包含用户名和密码, 格式为[name:password] [tom:123456]

-- fragment <默认true> 是否包含哈希值，格式为 #foo

-- search <默认true> 是否包含搜索查询的参数， 格式为 ?name=tom&age=100

-- unicode <默认false> 是否需要编码，对中文等Unicode 字符应该被直接编码而不是 Punycode 编码
...
三、MongoDB
1.关系型与非关系型数据库
image-20220413085332378image-20220413090707891
...
mongo
4.在命令行中操作数据库
image-20220413090814836image-20220413090825381image-20220413090837613image-20220413090858199image-20220413090907539image-20220413090916971
5.可视化工具进行增删改查
...
四、接口规范与业务分层
1.接口规范
image-20220414094020921image-20220414094043782
2.业务分层
